...
import type { LTWHP, ViewportHighlight } from "../types"; // your viewer's LTWHP + ViewportHighlight
import type { CommentedHighlight } from "../../app/src/types";
import { getHighlightColor } from "../../app/src/helpers/color";

export interface AreaHighlightProps {
  highlight: ViewportHighlight<CommentedHighlight>;   // <-- viewport highlight
  onChange?(rect: LTWHP): void;
  isScrolledTo?: boolean;
  bounds?: string | Element;
...
export const AreaHighlight: React.FC<AreaHighlightProps> = ({
  ...
  // This is ALREADY a viewport rect (LTWHP). Do NOT convert.
  const br: LTWHP = highlight.position.boundingRect;

  const color = getHighlightColor(highlight);

  // Force remount on rect change to keep Rnd in sync
  const key = `${br.width}-${br.height}-${br.left}-${br.top}`;
  ...
  return (
    ...
    >
      <Rnd
        ...
        key={key}
        bounds={bounds}
        default={{ x: br.left, y: br.top, width: br.width, height: br.height }}
